...
public class RedBlackTree {
    ...
    private Node rotate(int item, Node parent) {
        if (item < parent.element) {
            ...
            parent.left = item < node.element ? rotateWithLeftNode(node) : rotateWithRightNode(node);
            return parent.left;
        } else {
            Node node = parent.right;
            parent.right = item < node.element ? rotateWithLeftNode(node) : rotateWithRightNode(node);
            return parent.right;
        }
    }
